...
(* TODO: move to xenctrl *)
let string_of_policy_index = function
  | Xenctrl.Cpu_policy_raw -> "Raw"
...
let diff_bitset lhs rhs =
  (* rhs - lhs as feature bitsets = rhs & ~lhs *)
  Int64.(logand rhs (lognot lhs))
...
let get_policy_featureset xc name featureset index_default index_max =
  let fetch_policy_featureset index =
    ...
    debug "Retrieved %s policy from Xen: %s" (string_of_policy_index index) (Xenctrl.string_of_cpu_policy pol);
    pol
  in
  ...
  let featureset_default = Xenctrl.cpu_policy_to_featureset policy_default in
  let featureset_max = Xenctrl.cpu_policy_to_featureset policy_max in
  ...
  if not @@ Array.for_all (fun x -> Int64.equal x 0L) ours_minus_max then begin
    print_featureset (name ^ " ours minus max policy") ours_minus_max;
    warn "Our default policy has CPUID featureset that are not present in Max policy!";
  end;
  print_featureset (name ^ "max-default") (op_featureset diff_bitset featureset_default featureset_max);
  ...
  print_featureset (name ^ " migration_safe") migration_safe;
  migration_safe
